...
for ID = 5:6
    ...
    for smoothWay = "RTS"
        ...
        % LBL数据
        LBL =[d2r(truth(:,3:4)),-truth(:,5),truth(:,2)];
        ...
        % 根据设置是否启用平滑
        if SmoothIsOpen == 1
            navpath1 = fullfile(cfg.outputfolder, sprintf('Single-stage %s-LBL.nav', smoothWay));
            navfp1 = fopen(navpath1, 'wt');
            % 单次平滑结果
            navpath2 = fullfile(cfg.outputfolder, sprintf('Proposed two-stage %s-LBL.nav', smoothWay));
        ...
        %
        LBL = LBL(LBL(:, 4) >= cfg.starttime & LBL(:, 4) <= cfg.endtime, :);
        LBL(:, 1) = LBL(:, 1) + normrnd(0, 2/param.WGS84_RA, size(LBL(:, 1)));
        LBL(:, 2) = LBL(:, 2) + normrnd(0, 2/param.WGS84_RA, size(LBL(:, 2)));
        LBL(:, 3) = LBL(:, 3) + normrnd(0, 0.4, size(LBL(:, 3)));

        LBL = LBL(id*100:id*100:end,:);
        ...
        for imuindex = 2:size(imudata, 1)
            ...
            if lastimu(1, 1) == rangedata(rangeindex, 1) && cfg.userange==1
                ...
                kf = myLBLUpdate(navstate, LBL(rangeindex,:)', kf);
                rangeindex = rangeindex + 1;
