...
feedback = 1;
tic; % 启动计时器
height_updateway = 1;% 1为传统赋值方式，2为高度更新方式
for height_updateway = [0,1,2]
    rng(1);
    %% 导入数据
...
%% 对比不同高度更新方式
navFiles = {
    fullfile(cfg.outputfolder, 'Origin-rad-none.nav')
    fullfile(cfg.outputfolder, 'Origin-rad-assign.nav')
    fullfile(cfg.outputfolder, 'Origin-rad-measUpdate.nav')

    };

% path1 = "D:\Github\KF-GINS-Matlab\new_惯导试验\simu\output\Origin-rad-assign.nav";
% path2 = "D:\Github\KF-GINS-Matlab\new_惯导试验\simu\output\Origin-rad-measUpdate.nav";
% path0 = "D:\Github\KF-GINS-Matlab\new_惯导试验\simu\output\Origin-rad-none.nav";
...
% 保存图片
exportgraphics(fig1, savePrefix + ".png", "Resolution", 300);
