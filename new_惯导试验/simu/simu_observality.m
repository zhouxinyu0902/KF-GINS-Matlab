...
for smoothWay = "RTS"
    ...
    for mode = "Alternating"
        ...
        if SmoothIsOpen == 1
            % 二次平滑结果
            navpath1 = fullfile(cfg.outputfolder, sprintf('%s-DoubleSmooth-rad-%s.nav', smoothWay, mode));
        ...
        obslog = init_beacon_observability(beacon_positions, kf.P0, rngstd);
        ...
        for imuindex = 2:size(imudata, 1)
            ...
            elseif (lastimu(1, 1) < rangedata(rangeindex, 1) && thisimu(1, 1) > rangedata(rangeindex, 1))&& cfg.userange==1
                % OBSERVABILITY: this branch should ideally use a state propagated
                % exactly to the measurement time, consistent with the filter update.
                obslog = log_beacon_observation(obslog, ...
                    rangedata(rangeindex, 1), navstate.pos, ...
                    rangedata(rangeindex, 4:6)');
        ...
        % OBSERVABILITY: compare the actual alternating schedule with fixed-beacon
        % counterfactuals. Use at most nine measurements (three complete 1-2-3
        % cycles) so short experiments still provide multiple sliding windows.
        ...
        %         obslog, window_measurements, cfg.outputfolder);
        % else
        %     warning('Observability analysis skipped: only %d measurements available.', ...
        %         num_obs_events);
        % end
...
%%
pathcell={cfg.outputfolder+"/Origin-rad-alt.nav",...
...
exportgraphics(figall,"D:\WPS云盘\469639050\WPS云盘\成果\2_INS_RANGE\figs\simu_threewaysCMP.png", 'Resolution', 600);
% 
