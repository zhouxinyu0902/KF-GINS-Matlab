...
function plot_results(results, output_dir)
    ...
    legend(results.scenario_names, 'Location', 'best');
    saveas(fig, fullfile(output_dir, 'beacon_observability_comparison.png'));
    close(fig);
end

function plot_boxcharts(results, output_dir)
    num_scenarios = numel(results.scenario_names);
    group = repmat(1:num_scenarios, size(results.horizontal_lambda_min, 1), 1);
    group = group(:);
    ...
    boxplot(results.horizontal_lambda_min(:), group, ...
        'Labels', short_labels, 'LabelOrientation', 'inline');
    ylabel('\lambda_{min}(W_{horizontal})');
    title('Weakest horizontal information');
    grid on;
    ...
    saveas(fig, fullfile(output_dir, ...
...
function plot_metric(time, values, colors)
    ...
    for scenario_id = 1:size(values, 2)
        plot(time, values(:, scenario_id), 'LineWidth', 1.3, ...
