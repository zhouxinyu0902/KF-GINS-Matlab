...
%% 设置文件保存路径
navpath = fullfile(cfg.outputfolder, 'Origin-m.nav');
navfp = fopen(navpath, 'wt');

% 根据设置是否启用平滑
if SmoothIsOpen == 1
    % 二次平滑结果
    navpath1 = fullfile(cfg.outputfolder, sprintf('%s-DoubleSmooth-m.nav', smoothWay));
    navfp1 = fopen(navpath1, 'wt');
    % 单次平滑结果
    navpath2 = fullfile(cfg.outputfolder, sprintf('%s-SingleSmooth-m.nav', smoothWay));
...
for imuindex = 2:size(imudata, 1)
    ...
    if lastimu(1, 1) == rangedata(rangeindex, 1) && cfg.userange==1
        % 测量更新
        kf = myRangeUpdate_m(navstate, rangedata(rangeindex,:), height(imuindex,:), kf);
        rangeindex = rangeindex + 1;
        ...
        if SmoothIsOpen == 1
            if buf_idx > 1
                ...
                [nav_matrix, bridge_error, rtsstate_buffer] = perform_unified_smoothing(state_buffer, xk_final, ...
                    param, rangeindex, smoothWay, 'm', Pk_propa, Pk_k1propa, PHI);
                ...
                if isempty(prev_state_buffer)
                    ...
                    param, prev_rangeindex, smoothWay, 'm', prev_Pk_propa, prev_Pk_k1propa, prev_PHI);
    ...
    elseif (lastimu(1, 1) < rangedata(rangeindex, 1) && thisimu(1, 1) > rangedata(rangeindex, 1))&& cfg.userange==1
        ...
        % 调用高度卡尔曼量测更新
        kf = myHeightUpdate_m(navstate, height(imuindex, :), kf);
