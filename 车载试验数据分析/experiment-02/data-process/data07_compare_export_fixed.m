...
%
% 1. 三套IMU统一比较
%    -> 120 IMU / 830 binary IMU / 830 GPCHCX-derived IMU
%    -> 检查时间覆盖
%    -> FRD三轴/六轴标准化叠加
...
% 3. 航向
% 4. TXT导出
%    -> IMU直接导出第3部分确认正确的CompareIMU
%    -> 输出case-01 / case-02
% ========================================================================
%% 0. 参数
base_dir = 'D:\Github\KF-GINS-Matlab\data\experiment-data\experiment-02\1207-longtime';
...
ref830_file = fullfile(temp_dir,'ref830_1207_new.mat');
...
S = load(ref830_file);
if ~isfield(S,'Ref830')
    error('ref830_1207_new.mat 中没有 Ref830。');
end
Ref830 = S.Ref830;
...
nExp = min([numel(Compass),numel(Ref830),numel(Alignment120)]);
%% ========================================================================
%% 2. 数据结构检查
% ========================================================================
fprintf('\n============================================================\n');
...
for k = 1:nExp
    ...
    fprintf('Compass：%d 点，GPS %.3f ~ %.3f\n', ...
        length(Compass{k}.time),Compass{k}.time(1),Compass{k}.time(end));
    fprintf('830 PVA：%d × %d\n',size(Ref830{k}.pva,1),size(Ref830{k}.pva,2));
    if isfield(Ref830{k},'imu_gpchcx_frd') && ~isempty(Ref830{k}.imu_gpchcx_frd)
        fprintf('830 GPCHCX IMU：%d × %d\n', ...
            size(Ref830{k}.imu_gpchcx_frd,1),size(Ref830{k}.imu_gpchcx_frd,2));
    elseif isfield(Ref830{k},'imu') && ~isempty(Ref830{k}.imu)
        fprintf('830 GPCHCX IMU（旧字段imu）：%d × %d\n', ...
            size(Ref830{k}.imu,1),size(Ref830{k}.imu,2));
    else
        fprintf('830 GPCHCX IMU：不存在\n');
    end
    if isfield(Ref830{k},'imu_binary_frd') && ~isempty(Ref830{k}.imu_binary_frd)
        fprintf('830 binary IMU：%d × %d\n', ...
            size(Ref830{k}.imu_binary_frd,1),size(Ref830{k}.imu_binary_frd,2));
    else
        fprintf('830 binary IMU：不存在\n');
    end
    if isfield(Alignment120(k),'imu120FRD') && ~isempty(Alignment120(k).imu120FRD)
        fprintf('120 IMU：%d × %d\n', ...
            size(Alignment120(k).imu120FRD,1),size(Alignment120(k).imu120FRD,2));
    else
        fprintf('120 IMU：不存在\n');
    end
...
%% ========================================================================
%% 3. 120 / 830 binary / 830 GPCHCX IMU
% ========================================================================
fprintf('\n============================================================\n');
...
for k = 1:nExp
    ...
    %% 3.2 读取830 binary
    has_binary = isfield(Ref830{k},'imu_binary_frd') && ~isempty(Ref830{k}.imu_binary_frd);
    if has_binary
    ...
    has_gpchcx = isfield(Ref830{k},'imu_gpchcx_frd') && ~isempty(Ref830{k}.imu_gpchcx_frd);
    if has_gpchcx
    ...
    %% 3.4 提取时间和前三轴
    [t120,sig120] = prepare_imu_data(imu120,IMU_TIME_COL,IMU_FRD3_COLS);
    if has_binary
        [t_bin,sig_bin] = prepare_imu_data(imu830_binary,IMU_TIME_COL,IMU_FRD3_COLS);
    else
        t_bin = [];
        sig_bin = [];
    end
    if has_gpchcx
        [t_gpc,sig_gpc] = prepare_imu_data(imu830_gpchcx,IMU_TIME_COL,IMU_FRD3_COLS);
    else
        t_gpc = [];
        sig_gpc = [];
    end
    ...
    if has_binary
        idx_export_bin = imu830_binary(:,IMU_TIME_COL) >= common_start & ...
            imu830_binary(:,IMU_TIME_COL) <= common_end;
        CompareIMU{k}.imu830_binary = imu830_binary(idx_export_bin,:);
    else
        CompareIMU{k}.imu830_binary = [];
    end
    if has_gpchcx
        idx_export_gpc = imu830_gpchcx(:,IMU_TIME_COL) >= common_start & ...
            imu830_gpchcx(:,IMU_TIME_COL) <= common_end;
    ...
    %% 3.8 绘图索引
    id_plot_120 = plot_index(find(idx120),MAX_PLOT_POINTS);
    if has_binary
        id_plot_bin = plot_index(find(idx_bin),MAX_PLOT_POINTS);
    else
        id_plot_bin = [];
    end
    if has_gpchcx
        id_plot_gpc = plot_index(find(idx_gpc),MAX_PLOT_POINTS);
    else
        id_plot_gpc = [];
    end
    %% 图1：FRD前三轴
    figure('Name',sprintf('实验%d 三套IMU FRD时间对齐',k),'NumberTitle','off');
    tiledlayout(3,1,'TileSpacing','compact','Padding','compact');
    for axis_id = 1:3
        nexttile;
        hold on;
        grid on;
        plot(t120(id_plot_120)-common_start,sig120_std(id_plot_120,axis_id), ...
            'LineWidth',0.8,'DisplayName','120 IMU');
        if has_binary
            plot(t_bin(id_plot_bin)-common_start,sig_bin_std(id_plot_bin,axis_id), ...
                'LineWidth',0.8,'DisplayName','830 binary');
        end
        if has_gpchcx
            plot(t_gpc(id_plot_gpc)-common_start,sig_gpc_std(id_plot_gpc,axis_id), ...
                'LineWidth',0.8,'DisplayName','830 GPCHCX');
        end
        ylabel(axis_name_3{axis_id});
        if axis_id == 1
            title(sprintf('实验%d：120 / 830 binary / 830 GPCHCX FRD时间对齐',k));
    ...
    figure('Name',sprintf('实验%d 三套IMU机动特征',k),'NumberTitle','off');
    hold on;
    grid on;
    plot(t120(id_plot_120)-common_start,norm120(id_plot_120), ...
        'LineWidth',0.8,'DisplayName','120 IMU');
    if has_binary
        plot(t_bin(id_plot_bin)-common_start,norm_bin(id_plot_bin), ...
            'LineWidth',0.8,'DisplayName','830 binary');
    end
    if has_gpchcx
        plot(t_gpc(id_plot_gpc)-common_start,norm_gpc(id_plot_gpc), ...
            'LineWidth',0.8,'DisplayName','830 GPCHCX');
    end
    xlabel('共同运行时间 / s');
    ylabel('标准化FRD三轴模长');
    ...
    has_bin_6 = has_binary && size(imu830_binary,2) >= max(IMU_FRD6_COLS);
    has_gpc_6 = has_gpchcx && size(imu830_gpchcx,2) >= max(IMU_FRD6_COLS);
    if has_120_6 && has_bin_6 && has_gpc_6
        [t120_6,sig120_6] = prepare_imu_data(imu120,IMU_TIME_COL,IMU_FRD6_COLS);
        [tbin_6,sigbin_6] = prepare_imu_data(imu830_binary,IMU_TIME_COL,IMU_FRD6_COLS);
        [tgpc_6,siggpc_6] = prepare_imu_data(imu830_gpchcx,IMU_TIME_COL,IMU_FRD6_COLS);
        ...
        figure('Name',sprintf('实验%d 三套IMU六轴比较',k),'NumberTitle','off');
        tiledlayout(3,2,'TileSpacing','compact','Padding','compact');
        for axis_id = 1:6
            nexttile;
            hold on;
            grid on;
            plot(t120_6(id120_6)-common_start,sig120_6(id120_6,axis_id), ...
                'LineWidth',0.8,'DisplayName','120 IMU');
            plot(tbin_6(idbin_6)-common_start,sigbin_6(idbin_6,axis_id), ...
                'LineWidth',0.8,'DisplayName','830 binary');
            plot(tgpc_6(idgpc_6)-common_start,siggpc_6(idgpc_6,axis_id), ...
                'LineWidth',0.8,'DisplayName','830 GPCHCX');
        ...
        sgtitle(sprintf('实验%d：120 / 830 binary / 830 GPCHCX 六轴标准化比较',k));
    end
end
%% ========================================================================
%% 4. 830 / AUAX 轨迹
...
for k = 1:nExp
    pva830 = Ref830{k}.pva;
    lat830 = pva830(:,PVA_LAT_COL);
    lon830 = pva830(:,PVA_LON_COL);
    valid830 = isfinite(lat830) & isfinite(lon830) & lat830 ~= 0 & lon830 ~= 0;
    lat830 = lat830(valid830);
    lon830 = lon830(valid830);
    figure('Name',sprintf('实验%d 830-AUAX轨迹',k),'NumberTitle','off');
    hold on;
    grid on;
    axis equal;
    plot(lon830,lat830,'LineWidth',1.2,'DisplayName','830参考');
    plot(lon830(1),lat830(1),'*','MarkerSize',10,'DisplayName','830起点');
    has_auax = isfield(Alignment120(k),'auax') && ~isempty(Alignment120(k).auax);
    if has_auax
        A = Alignment120(k).auax;
        lat120 = A(AUAX_LAT_ROW,:);
        lon120 = A(AUAX_LON_ROW,:);
        valid120 = isfinite(lat120) & isfinite(lon120) & ...
            lat120 > 30 & lat120 < 40 & lon120 > 100 & lon120 < 130;
        lat120 = lat120(valid120);
        lon120 = lon120(valid120);
        if ~isempty(lat120)
            plot(lon120,lat120,'LineWidth',1,'DisplayName','120 AUAX');
            plot(lon120(1),lat120(1),'o','MarkerSize',8,'DisplayName','AUAX起点');
        end
    end
    xlabel('Longitude / °');
    ylabel('Latitude / °');
    title(sprintf('实验%d：830参考轨迹',k));
    legend('Location','best');
    if ~has_auax
        fprintf('实验%d AUAX轨迹缺失，本次仅显示830。\n',k);
    end
end
%% ========================================================================
%% 5. 120 / 830 / Compass 航向
% 横轴：绝对GPS周秒
% ========================================================================
fprintf('\n============================================================\n');
...
for k = 1:nExp
    t830 = Ref830{k}.time(:)';
    heading830 = mod(Ref830{k}.heading(:)',360);
    tCompass = Compass{k}.time(:)';
    headingCompass = mod(Compass{k}.heading(:)',360);
    has_auax = isfield(Alignment120(k),'auax') && ~isempty(Alignment120(k).auax);
    t_start = max(min(t830),min(tCompass));
    t_end = min(max(t830),max(tCompass));
    if has_auax
        A = Alignment120(k).auax;
        t120 = A(AUAX_TIME_ROW,:);
        heading120 = mod(-A(AUAX_HEADING_ROW,:),360);
        t_start = max(t_start,min(t120));
        t_end = min(t_end,max(t120));
    end
    if t_end <= t_start
        warning('实验%d航向数据没有有效公共区间。',k);
        continue;
    end
    idx830 = t830 >= t_start & t830 <= t_end;
    t = t830(idx830);
    h830 = heading830(idx830);
    hCompass = interp_heading_circular(tCompass,headingCompass,t);
    if has_auax
        h120 = interp_heading_gap(t120,heading120,t,AUAX_MAX_INTERP_GAP);
    else
        h120 = nan(size(t));
    end
    if has_auax
        valid = isfinite(h830) & isfinite(hCompass) & isfinite(h120);
    else
        valid = isfinite(h830) & isfinite(hCompass);
    end
    t_plot = t(valid);
    ...
    errorCompass = wrap180_local(hCompass_plot-h830_plot);
    if has_auax
        error120 = wrap180_local(h120_plot-h830_plot);
    end
    figure('Name',sprintf('实验%d 航向对比',k),'NumberTitle','off');
    tiledlayout(2,1,'TileSpacing','compact','Padding','compact');
    nexttile;
    hold on;
    grid on;
    plot(t_plot,h830_plot,'LineWidth',1.1,'DisplayName','830参考');
    plot(t_plot,hCompass_plot,'LineWidth',1,'DisplayName','罗盘');
    if has_auax
        plot(t_plot,h120_plot,'LineWidth',1,'DisplayName','120 AUAX');
    end
    ylabel('航向角 / °');
    xlabel('GPS周秒 / s');
    ylim([0 360]);
    xlim([t_plot(1),t_plot(end)]);
    title(sprintf('实验%d：航向角对比',k));
    legend('Location','best');
    ax = gca;
    ax.XAxis.Exponent = 0;
    xtickformat('%.0f');
    ...
    hold on;
    grid on;
    plot(t_plot,errorCompass,'LineWidth',1,'DisplayName','罗盘 - 830');
    if has_auax
        plot(t_plot,error120,'LineWidth',1,'DisplayName','120 - 830');
    end
    yline(0,'--');
    xlabel('GPS周秒 / s');
    ylabel('\Delta\psi / °');
...
valid = isfinite(t) & all(isfinite(sig),2);
t = t(valid);
sig = sig(valid,:);
[t,idx] = sort(t);
sig = sig(idx,:);
...
step = ceil(length(index)/max_points);
idx = index(1:step:end);
end
...
s = sind(h);
c = cosd(h);
si = interp1(t,s,tq,'linear',NaN);
ci = interp1(t,c,tq,'linear',NaN);
hq = mod(atan2d(si,ci),360);
hq = reshape(hq,tq_shape);
end
...
[t,idx] = unique(t,'stable');
h = h(idx);
hq_col = nan(size(tq_col));
if length(t) < 2
    hq = reshape(hq_col,tq_shape);
    return;
end
break_idx = find(diff(t) > max_gap);
seg_start = [1;break_idx+1];
seg_end = [break_idx;length(t)];
for i = 1:length(seg_start)
    ids = seg_start(i):seg_end(i);
    if length(ids) < 2
        continue;
    end
    tq_id = tq_col >= t(ids(1)) & tq_col <= t(ids(end));
    if ~any(tq_id)
        continue;
    end
    s = sind(h(ids));
    c = cosd(h(ids));
    si = interp1(t(ids),s,tq_col(tq_id),'linear');
    ci = interp1(t(ids),c,tq_col(tq_id),'linear');
    hq_col(tq_id) = mod(atan2d(si,ci),360);
end
hq = reshape(hq_col,tq_shape);
end
