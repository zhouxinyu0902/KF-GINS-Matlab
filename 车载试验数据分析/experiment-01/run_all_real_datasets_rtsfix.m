...
for input_id = input_ids
    ...
    in_dir = topic_paths.case_input(input_id);
    nav_out_dir = topic_paths.case_navigation(input_id);
    artifact_out_dir = topic_paths.case_artifacts(input_id);
    ...
    for unit_type = unit_types
      for solution_mode = solution_modes
        ...
        if unit_type == "rad"
            cfg = create_real_dataset_config( ...
                in_dir, unit_type, end_time_override);
        ...
        for imuindex = 2:size(imudata, 1)
            ...
            if lastimu(1, 1) == rangedata(rangeindex, 1) && cfg.userange == 1
                ...
                % 惯导推算
                % RTS 清空旧窗口后，Range 后的第一个 IMU 点仍需保存到新窗口。
                imudt = thisimu(1, 1) - lastimu(1, 1);
                if enable_smoothing == 1
            ...
            elseif lastimu(1) < rangedata(rangeindex, 1) && ...
                ...
                % 继续传播到当前 IMU 时刻，并将该点作为新 RTS 窗口首点。
                lastimu = firstimu;
