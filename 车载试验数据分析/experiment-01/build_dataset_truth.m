...
topic_root = script_dir;
addpath(topic_root);
topic_paths = setup_experiment_01();

dataset_ids = 1:8; % 可改为 1:8 批量构造
...
for dataset_id = dataset_ids
    ...
    %% 定义参数+加载过程配置
    param = Param();
    input_dir = topic_paths.case_input(dataset_id);

    cfg = create_real_dataset_config(input_dir, "rad");
    cfg.outputfolder = input_dir;
    %% 加载数据
    % imudata
    ...
    writematrix(gnssrtk, fullfile(input_dir, 'GNSS_RTK.txt'));


    gnssrtk_1s=gnssdata(1:100:end,:);
    gnssrtk_1s(:,2:3)=r2d(gnssrtk_1s(:,2:3));
    writematrix(gnssrtk_1s, fullfile(input_dir, 'GNSS_RTK_1s.txt'));
    % ==============================================================
    ...
    %% 设置文件保存路径
    navpath = fullfile(input_dir, 'truth.nav');
