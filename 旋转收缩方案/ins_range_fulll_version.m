...
%% 定义参数+加载过程配置
param = Param();
% cfg = ProcessConfigforSemiPhy();
% cfg = ProcessConfigforSemiPhy1();
cfg = ProcessConfigforSimu('旋转收缩方案/input_simu1');
cfg.outputfolder='旋转收缩方案/output_fullversion';
cfg.userange=1;
%% 加载数据
% imudata
...
if cfg.userange
    navpath = [cfg.outputfolder, '/RANGE',num2str(nnn),'-adap.nav'];
    disp("use RANGE data!");
else
...
% max_interv = 42;
navpath=[cfg.outputfolder,'/result-ForwardFilter.nav'];
navfp = fopen(navpath, 'wt');
navpath1=[cfg.outputfolder,'/result-BacwardInsmech.nav'];
navfp_fck =fopen(navpath1,'wt');
%
...
for imuindex = 2:size(imudata, 1)
    ...
    if lastimu(1, 1) == rangedata(rangeindex, 1)&& cfg.userange==1
        ...
        nav2save(:,imuindex) = [0;laststate_1.time;laststate_1.pos(1) * param.R2D; laststate_1.pos(2) * param.R2D; laststate_1.pos(3);...
                laststate_1.vel;laststate_1.att * param.R2D];
        ...
        for ii=indexrecord(ki)-1:-1:indexrecord(ki-1)
            ...
            nav2save(:,ii) = [0;laststate_1.time;laststate_1.pos(1) * param.R2D; laststate_1.pos(2) * param.R2D; laststate_1.pos(3);...
                laststate_1.vel;laststate_1.att * param.R2D];
        ...
        for iii=indexrecord(ki-1):indexrecord(ki)-1
            fprintf(navfp_fck, '%2d %12.6f %12.8f %12.8f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f \n', nav2save(:,iii));
        end
        % %% 一旦有测距信息就进行：反向推算+滤波
...
%%
path1=[cfg.outputfolder,'/RANGE1.nav'];
%%
nav11(:,imuindex:end)=[];
nav112(:,imuindex:end)=[];
...
pureinspath=[cfg.outputfolder,'/pureins', '.nav'];
calc_radial_error(cfg.truthpath,path1,navpath,output_file_back,output_file_rotateback)
hold on
...
%% 预备数据
refadappath=[cfg.outputfolder,'/RANGE1-adap.nav'];
refadappath0=[cfg.outputfolder,'/RANGE1-adap.nav'];
refpath='旋转收缩方案/output/RANGE.nav';
