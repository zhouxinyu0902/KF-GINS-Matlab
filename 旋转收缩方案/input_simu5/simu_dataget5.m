...
%%
ts = 0.01;  
avp0 = [[0;0;d2r(120)]; [0;0;0]; refavp(1,7:9)']; 
xxx = [];
seg = trjsegment(xxx, 'init',         0);
seg = trjsegment(seg, 'uniform',      m(2,1));
seg = trjsegment(seg, 'accelerate',   10, xxx, 0.20577); 
...
% 将原始坐标乘以 1000 (单位: 毫米)
dxyz_original = dxyz_relative * 1000;
center_mm = center * 1000; % 旋转中心也乘以 1000

% 定义旋转角度 (60度)
theta_deg = 60;
for theta_deg=0:30:180
...
% 对每个点应用以 center_mm 为中心的旋转
% 1. 平移到原点： P' = P - C
dxyz_shifted = dxyz_original' - repmat(center_mm, 1, size(dxyz_original, 1)); 

% 2. 旋转： P'' = R * P'
dxyz_rotated_shifted = R * dxyz_shifted;

% 3. 平移回原位： P_rotated = P'' + C
dxyz_rotated = (dxyz_rotated_shifted + repmat(center_mm, 1, size(dxyz_original, 1)))'; % 转置回原始行格式
...
output_file="D:\Github\KF-GINS-Matlab\旋转收缩方案\input_simu4\range1.txt";
try
...
output_file="D:\Github\KF-GINS-Matlab\旋转收缩方案\input_simu4\range2.txt";
try
...
output_file="D:\Github\KF-GINS-Matlab\旋转收缩方案\input_simu4\range3.txt";
try
...
%%
output_file="D:\Github\KF-GINS-Matlab\旋转收缩方案\input_simu4\IMU_120.txt";
try
...
% output_file="D:\Github\KF-GINS-Matlab\旋转收缩方案\input_simu4\IMU_120_005.txt";
% try
...
% output_file="D:\Github\KF-GINS-Matlab\旋转收缩方案\input_simu4\IMU_120_0003.txt";
% try
...
% output_file="D:\Github\KF-GINS-Matlab\旋转收缩方案\input_simu4\IMU_120_0005.txt";
% try
...
% end
% 
output_file="D:\Github\KF-GINS-Matlab\旋转收缩方案\input_simu4\truth.txt";
try
