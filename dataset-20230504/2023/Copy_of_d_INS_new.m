...
% 导入长基线、超短基线数据
load('2023\matData\LBL&USBL.mat')
%% 需要导出的数据
% 深度计
% IMU
% 长基线定位结果
...
pvaref=avpENU2NED([avp,Ins_Nav(end,:)']);
...
%% 轨迹对比
figure
plot(posLBL(9,:),posLBL(8,:),'.')
hold on
plot(Ins_Nav(2,:),Ins_Nav(1,:))
hold on
plot(posUSBL(10,:),posUSBL(9,:))
legend('LBL','INS-nav','USBL')
axis equal
%% 惯导解算
...
imu_NED=imuRFU2FRD(imu);
imupath="input/imu_NED.nav";
...
%% 插值后的深度保存
depth_intep=interp1(depth(end,:),depth(1,:),imu(:,end)','linear');
depth_intep=[depth_intep;imu(:,end)']';
path="input/depth-nav.nav";
fp=fopen(path,'wt');
fprintf(fp, '%.10f %.8f\n', depth_intep');
fclose(fp);
%% 
imupath="input/truth.nav";
imufp=fopen(imupath,'wt');
fprintf(imufp, '%.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f\n', pvaref');
fclose(imufp);
%%
ts=0.02;
avp_ins=[];
