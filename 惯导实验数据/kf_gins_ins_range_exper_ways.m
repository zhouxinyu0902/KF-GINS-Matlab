...
if cfg.userange
    navpath = [navpath, '-RANGE',num2str(nnn),'-adap'];
    disp("use RANGE data!");
end
navpath = [navpath, '.nav'];
...
for ki=2:12
    ...
    subplot(3,4,ki-1)
    plot(nav11(2,indexrecord(ki-1)+1:indexrecord(ki)), ...
        nav11(1,indexrecord(ki-1)+1:indexrecord(ki)))
    hold on
...
nav_backforward(:,3:4)=r2d(nav11(1:2,2:end)');
output_file_backforward = [navpath,'-backforward', '.txt'];
try
...
% 查看误差
calc_radial_error(cfg.truthpath,pureinspath, refpath, output_file_backforward)
hold on
...
output_file_rotate  =[navpath,'-rotate', '.txt'];
try
...
output_file_rotateback  =[navpath,'-rotate+backforward', '.txt'];
try
