...
%% 设置文件保存路径
navpath = fullfile(cfg.outputfolder, 'Origin.nav');
navfp = fopen(navpath, 'wt');

% 根据设置是否启用平滑
if SmoothIsOpen == 1
    % 二次平滑结果
    navpath1 = fullfile(cfg.outputfolder, sprintf('%s-DoubleSmooth.nav', smoothWay));
    navfp1 = fopen(navpath1, 'wt');
    % 单次平滑结果
    navpath2 = fullfile(cfg.outputfolder, sprintf('%s-SingleSmooth.nav', smoothWay));
