...
% range data
range1data = importdata(cfg.rangefile1path);
range1starttime = range1data(1, 1);
range1endtime = range1data(end, 1);

range2data = importdata(cfg.rangefile2path);
range2starttime = range2data(1, 1);
range2endtime = range2data(end, 1);

range3data = importdata(cfg.rangefile3path);
range3starttime = range3data(1, 1);
range3endtime = range3data(end, 1);
...
range1data = range1data(range1data(:, 1) >= cfg.starttime, :);
range1data = range1data(range1data(:, 1) <= cfg.endtime, :);
range2data = range2data(range2data(:, 1) >= cfg.starttime, :);
range2data = range2data(range2data(:, 1) <= cfg.endtime, :);
range3data = range3data(range3data(:, 1) >= cfg.starttime, :);
range3data = range3data(range3data(:, 1) <= cfg.endtime, :);
heightdata = heightdata(heightdata(:, 1) >= cfg.starttime, :);
heightdata = heightdata(heightdata(:, 1) <= cfg.endtime, :);

% dt=0.5/0.005;
% heightdata = heightdata(1:dt:end,:);% 深度计的频率
% dt=1/0.005;
% n=floor(length(range1data)/dt);
% rangedata = zeros(n,6);
% rangedata(1:3:end,:) = range1data(1:dt*3:end-1,:);
% rangedata(2:3:end,:) = range2data(2:dt*3:end-1,:);
% rangedata(3:3:end,:) = range3data(3:dt*3:end,:);
%% for debug
...
for imuindex = 2:size(imudata, 1)-1
    ...
    %     rangeindex = rangeindex + 1;
    % end
    % % check whether rangedata data is valid
    ...
    % only do propagation
        ...
        % navstate.vel(3) = 0;% 天向速度置零
        % error propagation
        kf = myInsPropagate_15state(navstate, thisimu, imudt, kf, cfg.corrtime);
    % end
