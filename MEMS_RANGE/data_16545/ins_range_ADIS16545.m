...
glvs
%%
% imu120=importdata("旋转收缩方案1\input2\IMU_120.txt");
% imu830=importdata("旋转收缩方案1\input2\imu_830.txt");
% imu120=imuFRD2RFU(imu120);
% imu830=imuFRD2RFU(imu830);
% imuplot(imu120)
% imuplot(imu830)
%% 定义参数+加载过程配置
param = Param();
path='D:\Github\KF-GINS-Matlab\MEMS_RANGE\data_16545';
cfg = ProcessConfig_16545(path);
get3beacons(path)
% close all
%% 加载数据
% imudata
...
% rangedata = importdata(cfg.rangefilemovingpath);
...
heightdata(:,1) = imudata(:, 1);
heightdata(:,2) = interp1(height(:,1), height(:,2),heightdata(:,1), 'spline');
heightdata(:,2) = heightdata(:,2) + normrnd(0,depstd,size(heightdata(:,2)));
heightstarttime = heightdata(1, 1);
heightendtime = heightdata(end, 1);

%% 获取处理时间，调整时间
if imustarttime > heightstarttime
    starttime = imustarttime;
else
    starttime = heightstarttime;
end
if imuendtime > heightendtime
    endtime = heightendtime;
else
    endtime = imuendtime;
end
...
% plot(truth(1,4),truth(1,3),'*')
% hold on
% plot(r2d(rangedata(:,5)),r2d(rangedata(:,4)))
% legend('轨迹','起点','信标')
% xygo('经度/°','纬度/°')
% % title('仿真信标及仿真轨迹')
% 
...
for type ="EKF"
    ...
    for imuindex = 2:size(imudata, 1)
        ...
        if lastimu(1, 1) == rangedata(rangeindex, 1)&& cfg.userange==1
            ...
            depthdata = heightdata(imuindex,1:2);
            if cfg.adap==1
        ...
        elseif (lastimu(1, 1) < rangedata(rangeindex, 1) && thisimu(1, 1) > rangedata(rangeindex, 1))&& cfg.userange==1
            ...
            % INS mechanization
            navstate = InsMech(laststate, lastimu, thisimu);
            navstate.pos(3) = heightdata(imuindex,2) ; % 天向位置约束
            % error propagation
            kf = myInsPropagate_15state(navstate, thisimu, imudt, kf);
        end
...
%%
% path_EKF=[cfg.outputfolder,'/MEMS-EKF.nav'];
% calc_error(path_EKF,cfg.truthpath)
plot_result(navpath)
...
num=floor(0.1/(truth(2,2)-truth(1,2)));
GNSS_1s = truth(1:num:end,2:5);% 时间间隔
