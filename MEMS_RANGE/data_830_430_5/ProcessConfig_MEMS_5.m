...
function cfg = ProcessConfig_MEMS_5(path)
    param = Param();
    %% filepath
    type=830;
    if type==430
        cfg.imufilepath = [path,'/imu_430.txt'];
    else
        cfg.imufilepath = [path,'/imu_830.txt'];
    end
    
    % cfg.imufilepath = [path,'/IMU_120.txt'];
    cfg.gnssfilepath = [path,'/pva_830.txt'];
    cfg.rangefile1path = [path,'/range1.txt'];
