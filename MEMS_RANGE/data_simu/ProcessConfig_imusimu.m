...
function cfg = ProcessConfig_imusimu(path)
    param = Param();
    %% path
    cfg.truthpath = [path,'\truth.nav'];
    cfg.imufilepath = [path,'\imu.nav'];
    cfg.gnssfilepath = [path,'\gnss.txt'];
    cfg.depthfilepath = [path,'\height-markov-10Hz.txt'];
    cfg.LBLfilepath = [path,'\LBL.txt'];
    cfg.outputfolder = path;
    for i = 1:9
        fieldname = sprintf('rangefile%dpath', i);
        filename = sprintf('range%d.txt', i);
        cfg.(fieldname) = fullfile(path, filename);
    end
    cfg.rangefilemovingpath = [path,'/range_moving.txt'];
    cfg.singlerangefilepath = [path,'/single_range.txt'];    
    % cfg.odofilepath = '';
    %% configure
