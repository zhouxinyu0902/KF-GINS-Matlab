%% 构造标准参考轨迹与 GNSS-RTK 派生数据
% 原始输入仍从 F 盘读取，所有生成文件写入总 data 目录。
clear; clc; close all;
...
writematrix(gnssrtk, fullfile(derived_dir, 'GNSS_RTK.txt'));


gnssrtk_1s=gnssdata(1:100:end,:);
gnssrtk_1s(:,2:3)=r2d(gnssrtk_1s(:,2:3));
writematrix(gnssrtk_1s, fullfile(derived_dir, 'GNSS_RTK_1s.txt'));
% ==============================================================
...
%% 设置文件保存路径
navpath = fullfile(derived_dir, 'truth.nav');
navfp = fopen(navpath, 'wt');
if navfp < 0
    error('无法创建 truth 文件：%s', navpath);
end
nav_cleanup = onCleanup(@() close_nav_files(navfp));
...
fprintf('Dataset %d truth 已保存至：%s\n', dataset_id, navpath);
end
