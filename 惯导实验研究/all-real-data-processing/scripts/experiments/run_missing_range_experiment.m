%% 测距点缺失敏感性实验
% 默认针对第 6 组实测数据，分别移除第 1/3/5/7/9/11 个测距点。
clear; clc; close all;
...
glvs
dataset_id = 6;
input_dir = fullfile(topic_paths.external_input_root, ...
    ['input', num2str(dataset_id)]);
derived_dir = fullfile(topic_paths.derived_input, ...
    ['dataset', num2str(dataset_id)]);
nav_output_dir = fullfile(topic_paths.navigation_results, ...
    ['dataset', num2str(dataset_id)], 'missing-range');
artifact_output_dir = fullfile(topic_paths.figures_tables, ...
    ['dataset', num2str(dataset_id)], 'missing-range');
if ~exist(nav_output_dir, 'dir'), mkdir(nav_output_dir); end
if ~exist(artifact_output_dir, 'dir'), mkdir(artifact_output_dir); end

cfg = create_real_dataset_config(input_dir, derived_dir, "rad", []);
cfg.outputfolder = nav_output_dir;
rangstd = 6;
depthstd = 0.4;
rng(1)
...
test_cases = {
    ...
    1,  'Drop-1';          % 场景2: 仅丢失第 1 个点
    3,  'Drop-3';          % 场景3: 仅丢失第 3 个点
    5,  'Drop-5';          % 场景4: 仅丢失第 5 个点
    7,  'Drop-7';          % 场景5: 仅丢失第 7 个点
    9,  'Drop-9';          % 场景6: 仅丢失第 9 个点
    11, 'Drop-11';         % 场景7: 仅丢失第 11 个点
};
...
for case_idx = 1:size(test_cases, 1)
    ...
    navpath = fullfile(cfg.outputfolder, sprintf('Origin-%s.nav', case_name));
    navfp = fopen(navpath, 'wt');
    if SmoothIsOpen == 1
        navpath1 = fullfile(cfg.outputfolder, ...
            sprintf('RTS-DoubleSmooth-%s.nav', case_name));
        navfp1 = fopen(navpath1, 'wt');
        navpath2 = fullfile(cfg.outputfolder, ...
            sprintf('RTS-SingleSmooth-%s.nav', case_name));
        ...
        if rangeindex <= size(rangedata, 1) && lastimu(1, 1) == rangedata(rangeindex, 1) && cfg.userange==1
            ...
            if SmoothIsOpen == 1
                if buf_idx > 1
                    ...
                    elseif strcmp(smoothWay,'Linear') %#ok<UNRCH>
...
%%
ii = [1,3,5,7,9,11];
navpath = fullfile(nav_output_dir, 'Origin-Baseline.nav');
navph = cell(1, numel(ii));
for i=1:6
    navph{i} = fullfile(nav_output_dir, ...
        ['Origin-Drop-', num2str(ii(i)), '.nav']);
end
[fig,finalExcelData]=calc_radial_error(cfg.truthpath,navpath,navph{:});
outputExcelName = fullfile(artifact_output_dir, ...
    '导航系统径向误差统计报告-all-loserange.xlsx');
writecell(finalExcelData, outputExcelName);
exportgraphics(fig, fullfile(artifact_output_dir, 'loserange.png'), ...
    'Resolution', 600);
