function cfg = create_real_dataset_config(input_dir, derived_dir, position_unit, end_time_override)
%CREATE_REAL_DATASET_CONFIG 构造全实测数据处理专题的统一配置。
% input_dir 保持指向 F 盘原始数据；derived_dir 保存 truth 等派生输入。
% position_unit 可取 "rad" 或 "m"，仅决定水平位置误差状态的单位。

    if nargin < 2 || isempty(derived_dir)
        derived_dir = input_dir;
    end
    if nargin < 3 || isempty(position_unit)
        position_unit = "rad";
    end
    if nargin < 4
        end_time_override = [];
    ...
    derived_truth = fullfile(derived_dir, 'truth.nav');
    source_truth = fullfile(input_dir, 'truth.nav');
    if isfile(derived_truth)
        cfg.truthpath = derived_truth;
    else
        cfg.truthpath = source_truth;
    end
    ...
    cfg.endtime = pva_830(initial_index, 2)+4621;
