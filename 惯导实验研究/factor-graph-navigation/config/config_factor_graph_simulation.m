function cfg = config_factor_graph_simulation(in_dir)
%CONFIG_FACTOR_GRAPH_SIMULATION 因子图仿真数据集的统一导航配置。
    param = Param();
    ...
    %% initial information
    pva_120 = importdata(cfg.truthpath);
    % 选择计算时间段
    id = 1;
    cfg.starttime = pva_120(id,2);
    cfg.endtime = pva_120(end,2);
    % cfg.endtime = cfg.starttime + 5000;
    % 初始状态

    cfg.initpos = pva_120(id,3:5)'; % [deg, deg, m]
