...
function cfg = ProcessConfig_imusimu(path)
    param = Param();
    %% path
    input_dir = fullfile(path, 'input');
    cfg.truthpath = fullfile(input_dir, 'truth.nav');
    cfg.imufilepath = fullfile(input_dir, 'imu.nav');
    cfg.gnssfilepath = fullfile(input_dir, 'gnss.txt');
    cfg.depthfilepath = fullfile(input_dir, 'height-100Hz.txt');
    cfg.LBLfilepath = fullfile(input_dir, 'LBL.txt');
    cfg.outputfolder = fullfile(path, 'output');
    if ~isfolder(cfg.outputfolder)
        mkdir(cfg.outputfolder);
    end
    for i = 1:9
        fieldname = sprintf('rangefile%dpath', i);
        filename = sprintf('range%d.txt', i);
        cfg.(fieldname) = fullfile(input_dir, filename);
    end
    cfg.rangefilemovingpath = fullfile(input_dir, 'range_moving.txt');
    cfg.singlerangefilepath = fullfile(input_dir, 'single_range.txt');
    % cfg.odofilepath = '';
    %% configure
    ...
    cfg.accvrw = 0.018; % [m/s/sqrt(h)] 加速度计随机游走
