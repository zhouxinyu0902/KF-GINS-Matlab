...
%% 获取处理时间，调整时间
starttime = max(imustarttime, gnssstarttime);
endtime = min(imuendtime, gnssendtime);
...
height_values = interp1(gnssdata(:, 1), gnssdata(:, 4), ...
    imudata(:, 1), 'linear', 'extrap');

%% 设置文件保存路径
navpath = [path,'/truth_gen.nav'];
...
for imuindex = 2:size(imudata, 1)
    ...
    if lastimu(1, 1) == gnssdata(gnssindex, 1)
        ...
        navstate.pos(3) = height_values(imuindex);
