function outputs = simulate_navigation_case(case_name, result_dir, compute_auxiliary_results,enable_fixed_lag_replay, runtime_options)
%SIMULATE_NAVIGATION_CASE 可复用的单数据集扩展仿真引擎。
%   outputs = simulate_navigation_case(case_name, result_dir, false)
%   用于三方法批量对比，只保留 EKF/RTS 主链，跳过 BRC 辅助算法。
%   第五个输入 runtime_options 用于选择位置误差状态单位、注入量测更新
%   函数或替换三个测距源；省略时默认采用 rad 链，与原程序一致。

%% 用户配置：直接点击“运行”时在这里修改默认数据集和运行内容
...
if nargin < 5 || isempty(runtime_options)
    runtime_options = struct();
end
if ~isstruct(runtime_options)
    error('runtime_options 必须是结构体。');
end
...
options.random_seed = get_runtime_option(runtime_options, 'random_seed', 1);
options.range_interval_s = get_runtime_option( ...
    runtime_options, 'range_interval_s', 420);
options.beacon_order = get_runtime_option( ...
    runtime_options, 'beacon_order', [1, 2, 3]);
options.range_noise_std_m = get_runtime_option( ...
    runtime_options, 'range_noise_std_m', 10);
options.depth_noise_std_m = get_runtime_option( ...
    runtime_options, 'depth_noise_std_m', 0.4);
options.filter_range_std_m = get_runtime_option( ...
    runtime_options, 'filter_range_std_m', 10);
options.filter_depth_std_m = get_runtime_option( ...
    runtime_options, 'filter_depth_std_m', 0.4);
options.use_adaptive_forward_update = logical(get_runtime_option( ...
    runtime_options, 'use_adaptive_forward_update', true));
...
options.end_time_s = get_runtime_option( ...
    runtime_options, 'end_time_s', 4621);
...
options.measurement_update_function = get_runtime_option( ...
    runtime_options, 'measurement_update_function', []);
options.error_feedback_function = get_runtime_option( ...
    runtime_options, 'error_feedback_function', []);
options.range_source_paths = get_runtime_option( ...
    runtime_options, 'range_source_paths', {});
options.range_data = get_runtime_option(runtime_options, 'range_data', []);
options.position_error_unit = lower(string(get_runtime_option( ...
    runtime_options, 'position_error_unit', 'rad')));
if ~ismember(options.position_error_unit, ["rad", "m"])
    error('position_error_unit 只能设置为 "rad" 或 "m"。');
end
if ~isscalar(options.random_seed) || ~isfinite(options.random_seed)
    error('random_seed 必须是有限标量。');
end
if options.range_interval_s <= 0 || options.end_time_s <= 0 || ...
        options.range_noise_std_m < 0 || options.depth_noise_std_m < 0 || ...
        options.filter_range_std_m <= 0 || options.filter_depth_std_m <= 0
    error('时间间隔和滤波标准差必须为正，注入噪声标准差不能为负。');
end
if isempty(options.beacon_order) || ...
        any(~ismember(options.beacon_order, 1:3))
    error('beacon_order 只能由 1、2、3 组成。');
end
rng(options.random_seed, 'twister'); % 所有工况使用同一随机序列，便于控制变量。
...
if nargin < 2 || isempty(result_dir)
    result_dir = fullfile(project_root, 'data', 'inertial-experiment', ...
        ...
        options.case_name, sprintf('navigation-comparison-%s', ...
...
if isempty(options.range_data)
    if isempty(options.range_source_paths)
        range_source_paths = {cfg.rangefile1path, cfg.rangefile2path, ...
            cfg.rangefile3path};
    else
        range_source_paths = cellstr(string(options.range_source_paths));
        if numel(range_source_paths) ~= 3
            error('range_source_paths 必须包含三个测距文件。');
        end
        missing_mask = ~cellfun(@isfile, range_source_paths);
        if any(missing_mask)
            error('找不到测距文件：%s', ...
                strjoin(range_source_paths(missing_mask), ', '));
        end
    end
    range_sources = cellfun(@(path) readmatrix(path, 'FileType', 'text'), ...
        range_source_paths, 'UniformOutput', false);
    ...
    rangedata = double(options.range_data);
    range_source_paths = {};
    if isempty(rangedata) || size(rangedata, 2) < 6 || ...
            any(diff(rangedata(:, 1)) <= 0)
        error('range_data 必须是时间严格递增的非空 N×6（或更多列）矩阵。');
    end
end
...
for imu_index = 2:sample_count
    ...
    if is_range_epoch
        ...
        kf = update_range_filter(navstate, current_range, current_depth, kf, ...
            ...
            options.measurement_update_function, options.position_error_unit);
        terminal_error = kf.x;
        ...
        [single_segment_nav, bridge_error, first_error_nodes] = ...
            ...
            rts_segment_indices, param, options.position_error_unit);
        ...
        if ~isempty(previous_rts_block)
            ...
            rts_diagnostics(previous_index).second_pass_terminal_error_m = ...
                ...
                previous_rts_block.end_position, options.position_error_unit);
            ...
            if ~isempty(previous_double_block)
                ...
                rotation_item.residual_bridge_error_m = ...
                    ...
                    target_position, options.position_error_unit);
        ...
        rts_info.terminal_error_m = horizontal_error_norm( ...
            terminal_error, navstate.pos, options.position_error_unit);
        ...
        if isempty(options.error_feedback_function)
            if options.position_error_unit == "rad"
            ...
            if ~isa(options.error_feedback_function, 'function_handle')
                error('error_feedback_function 必须是函数句柄。');
            end
            [kf, navstate] = options.error_feedback_function(kf, navstate);
        end
    ...
    if ~is_range_epoch
        if options.position_error_unit == "rad"
            ...
            [kf, navstate, height_transition] = update_decoupled_height_m( ...
                kf, navstate, height(imu_index, :));
        end
    end
    if options.position_error_unit == "rad"
...
outputs = struct( ...
    ...
    'input_dir', input_dir, ...
    'range_source_paths', {range_source_paths}, ...
    'result_dir', cfg.outputfolder, ...
    'forward_path', forward_path, ...
    'single_rts_path', rts_single_path, ...
    'double_rts_path', rts_double_path, ...
    'rotation_contraction_path', rts_rotation_path, ...
    'forward_mask', all(isfinite(forward_nav(:, 2:5)), 2), ...
    'single_rts_mask', rts_single_is_processed, ...
    'double_rts_mask', rts_double_is_processed, ...
    'rotation_contraction_mask', rts_rotation_is_processed, ...
...
function kf = update_range_filter(navstate, range_data, depth_data, kf, ...
        use_adaptive, measurement_update_function, position_error_unit)
    if ~isempty(measurement_update_function)
        if ~isa(measurement_update_function, 'function_handle')
            error('measurement_update_function 必须是函数句柄。');
        end
        kf = measurement_update_function(navstate, range_data, depth_data, kf);
    elseif position_error_unit == "m"
        % 当前专题没有米制自适应距离更新，m 链使用标准 EKF 更新。
...
function value = get_runtime_option(options, field_name, default_value)
...
% 历元，仅改变输出采样密度，不改变 RTS 节点上的递推关系。
    ...
    smoothed_nav = apply_error_to_nav( ...
        nominal_nav, full_error, param, position_error_unit);
    bridge_error = error_nodes(1, :)';
end

function corrected_nav = apply_error_to_nav( ...
        nominal_nav, error_state, param, position_error_unit)
%APPLY_ERROR_TO_NAV 按当前 15 状态滤波器的反馈符号修正导航输出。
    ...
    if position_error_unit == "rad"
        ...
        % m 链前三维为 [N,E,D]。名义位置仍以 [rad,rad,m] 保存，
        % 因而逐历元转换回大地坐标后再写入 nav 文件。
        for row_index = 1:size(nominal_nav, 1)
            nominal_position = [nominal_nav(row_index, 3:4)' * param.D2R; ...
                nominal_nav(row_index, 5)];
            [rm, rn] = getRmRn(nominal_position(1), param);
            dr_inverse = diag([1 / (rm + nominal_position(3)), ...
                1 / ((rn + nominal_position(3)) * cos(nominal_position(1))), ...
                -1]);
            corrected_position = nominal_position ...
                - dr_inverse * error_state(row_index, 1:3)';
            corrected_nav(row_index, 3:5) = [ ...
                corrected_position(1:2)' * param.R2D, corrected_position(3)];
        end
    end
...
function value = horizontal_error_norm( ...
        error_state, position, position_error_unit)
...
function [kf, navstate] = feedback_fixed_lag_error( ...
