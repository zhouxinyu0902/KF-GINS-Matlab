...
function cfg = ProcessConfig_truth(path)
    ...
    output_root = fullfile(project_root, 'data', 'inertial-experiment', ...
        'algorithm-exploration', 'navigation-results', 'simulation', 'truth-cases');
    if path(end)=='1'
        case_name = 'case-01';
    elseif path(end)=='2'
        case_name = 'case-02';
    ...
    elseif path(end)=='m'
        ...
        error('Cannot infer truth-case name from path: %s', path);
    end
    cfg.outputfolder = fullfile(output_root, case_name);
