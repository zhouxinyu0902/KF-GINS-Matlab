...
output_path = fullfile(result_dir, ...
    'range-ins-double-rts-position-guided-replay.nav');
artifact_dir = exploration_artifact_dir(result_dir);
update_diagnostic_path = fullfile(artifact_dir, ...
    'range-ins-double-rts-position-guided-replay-updates.csv');
statistics_path = fullfile(artifact_dir, ...
    'range-ins-double-rts-position-guided-replay-statistics.csv');
figure_path = fullfile(figure_dir, ...
    'range-ins-double-rts-position-guided-replay-comparison.png');
figure_source_path = fullfile(figure_dir, ...
    'range-ins-double-rts-position-guided-replay-comparison.fig');

required_files = {cfg.imufilepath, cfg.truthpath, cfg.rangefile1path, ...
    cfg.rangefile2path, cfg.rangefile3path, double_rts_path, forward_path};
...
for imu_index = 2:sample_count
    ...
    if last_imu(1) >= next_update_time - time_tolerance
        measurement_position = interp1(double_rts_nav(:, 2), ...
            double_rts_nav(:, 3:4), last_imu(1), 'linear')' * param.D2R;
        [position_std_m, is_boundary_guard] = select_position_std( ...
            last_imu(1), options);
        [kf, update_info] = update_horizontal_rts_position( ...
            navstate, measurement_position, kf, position_std_m, options);

        if update_info.accepted
            [kf, navstate] = feedback_replay_error(kf, navstate);
...
%% 统一评价前向 EKF、二次 RTS和重放结果
truth_position = interp1(truth_all(:, 2), truth_all(:, 3:5), ...
    imudata(:, 1), 'linear', 'extrap');
method_names = {'前向 EKF', '二次 RTS', '2RTS约束INS重放'};
nav_results = {forward_nav, double_rts_nav, replay_nav};
...
colors = [0.10, 0.35, 0.75; 0.05, 0.55, 0.55; 0.80, 0.20, 0.45];
line_styles = {'-', '--', '-.'};

comparison_figure = figure('Color', 'w', ...
    'Name', '二次 RTS约束的历史 IMU 重放', ...
    'Position', [80, 120, 1500, 600]);
layout = tiledlayout(comparison_figure, 1, 2, ...
    'TileSpacing', 'compact', 'Padding', 'compact');
title(layout, sprintf(['%s：2RTS约束历史IMU重放', ...
    '（伪量测间隔 %.0f s，标准差 %.0f m）'], ...
...
%% 局部函数
function [position_std_m, is_boundary_guard] = ...
        select_position_std(time_s, options)
%SELECT_POSITION_STD 在二次 RTS 分段交界附近连续降低伪量测权重。
...
function [kf, info] = update_horizontal_rts_position( ...
        navstate, measurement_position, kf, position_std_m, options)
%UPDATE_HORIZONTAL_RTS_POSITION 使用二次 RTS 纬经度更新水平位置误差状态。
    horizontal_scale = [navstate.Rm + navstate.pos(3); ...
        (navstate.Rn + navstate.pos(3)) * cos(navstate.pos(1))];
    measurement_residual = navstate.pos(1:2) - measurement_position;
    state_matrix = zeros(2, kf.RANK);
    state_matrix(:, 1:2) = eye(2);
    innovation = measurement_residual - state_matrix * kf.x;
    innovation_m_vector = horizontal_scale .* innovation;
    innovation_m = norm(innovation_m_vector);

    effective_std_m = position_std_m;
    normalized_innovation = innovation_m / max(position_std_m, eps);
...
function [kf, navstate] = feedback_replay_error(kf, navstate)
...
function item = empty_update_diagnostic()
    item = struct('time_s', nan, 'position_std_m', nan, ...
        'in_boundary_guard', false, 'innovation_m', nan, ...
