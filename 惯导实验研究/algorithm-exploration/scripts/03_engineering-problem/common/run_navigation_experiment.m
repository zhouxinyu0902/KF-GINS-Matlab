function outputs = run_navigation_experiment(result_dir, runtime_options)
%RUN_NAVIGATION_EXPERIMENT 实测导航主程序（rad位置误差状态）。
%   本文件按“用户配置—数据读取—主循环—结果保存”组织完整算法，默认
%   无输入即可运行；延迟等专题可通过 runtime_options 复用同一主程序。
...
options.filter_range_std_m = option_value( ...
    runtime_options, 'filter_range_std_m', 6);
options.filter_depth_std_m = option_value( ...
...
% 测距时刻守恒拆分 IMU 增量，使量测更新严格发生在对应时刻。RTS、
% 固定滞后重放和结果数组仍复用下面唯一一套主循环。
