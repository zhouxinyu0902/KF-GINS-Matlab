...
if imuerrrecord == 1
    imuerrpath = [cfg.figurefolder, '/NavResult-gnss-ImuError.txt'];
    imuerrfp = fopen(imuerrpath, 'wt');
end
if stdrecord == 1
    stdpath = [cfg.figurefolder, '/NavSTD.txt'];
    stdfp = fopen(stdpath, 'wt');
end
if feedback == 0
    xkpath = [cfg.figurefolder, '/xk-gnss.txt'];
...
for imuindex = 2:size(imudata, 1)
    ...
    elseif (lastimu(1, 1) < gnssdata(gnssindex, 1) && thisimu(1, 1) > gnssdata(gnssindex, 1))
        ...
        % 插值到 GNSS 时刻前同样采用解耦高度 Kalman 更新。
        ...
        %% only do propagation
        navstate = InsMech(laststate, lastimu, thisimu);
        % 与 all_m.m 一致：每个非 GNSS 历元只更新并反馈垂向位置、速度。
        kf = myHeightUpdate(navstate, heightdata(imuindex, :), kf);
        navstate.pos(3) = navstate.pos(3) - kf.x(3);
        navstate.vel(3) = navstate.vel(3) - kf.x(6);
        kf.x(3) = 0;
...
fprintf('导航结果：%s\n', navpath);
fprintf(['如需误差评价，请运行 scripts/evaluation/gnss-ins/', ...
    'evaluate_fgo_gnss_ins.m。\n']);
