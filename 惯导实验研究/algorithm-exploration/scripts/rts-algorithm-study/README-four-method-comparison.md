...
```text
data/inertial-experiment/algorithm-exploration/navigation-results/simulation/<case>/four-method-comparison-<rad|m>
```

其中包含四种导航结果、旋转收缩诊断、位置速度伪量测诊断、误差统计，以及轨迹和水平径向误差对比图。所有数据集的统计汇总保存为：

```text
data/inertial-experiment/algorithm-exploration/figures-tables/simulation/four-method-comparison-<rad|m>-summary.csv
```
