...
for case_index = 1:numel(selected_cases)
    ...
    result_dir = fullfile(output_root, case_name, ...
        sprintf('fixed-lag-four-method-comparison-%s', ...
        position_error_unit));
    ...
    try
        ...
        if overwrite_existing || ~results_ready
            ...
            outputs = simulate_navigation_case( ...
                case_name, result_dir, false, true, runtime_options);
...
if ~isempty(all_statistics)
    summary_path = fullfile(exploration_artifact_dir(output_root), ...
        sprintf('fixed-lag-four-method-comparison-%s-summary.csv', ...
        position_error_unit));
