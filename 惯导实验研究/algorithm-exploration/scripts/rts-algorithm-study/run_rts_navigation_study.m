...
%% RTS算法统一研究主程序：前向 ES-EKF 与可选的一次、二次 RTS
% 算法主循环只有一份；data_source 负责在仿真和实测输入适配器之间切换。
% 配置、输入和输出随数据来源变化，EKF/RTS处理顺序保持完全相同。
%
% 对比结果：
%   1）前向 ES-EKF；
%   2）一次分段 RTS；
...
if data_source == "simulation"
    ...
    output_dir = fullfile(paths.simulation_navigation, case_name, ...
        sprintf('simple-ekf-rts-%s', position_error_unit));
    ...
    output_dir = fullfile(paths.experiment_navigation, ...
        sprintf('simple-ekf-rts-%s', position_error_unit));
...
if data_source == "simulation"
    ...
    % 三个1 Hz距离源按7 min抽取，再按照1→2→3轮换并加入仿真噪声。
...
if data_source == "simulation"
    ...
    height = [imudata(:, 1), height_value + ...
        simulation_depth_noise_std_m * randn(size(height_value))];
else
...
last_progress = -1;
fprintf(['开始处理 %s/%s（%s位置误差状态）：前向EKF=%d，', ...
    '一次RTS=%d，二次RTS=%d。\n'], ...
    data_source, case_name, position_error_unit, enable_feedback, enable_smoothing, ...
    enable_smoothing && enable_second_rts);
tic;

%% 6. 主循环：惯导传播，在测距点执行更新和分段 RTS
