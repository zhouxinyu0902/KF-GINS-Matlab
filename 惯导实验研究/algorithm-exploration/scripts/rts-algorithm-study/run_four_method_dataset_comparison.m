...
%
% 输出目录按输入数据集和误差状态单位自动隔离：
%   .../simulation/<case>/four-method-comparison-rad
%   .../simulation/<case>/four-method-comparison-m
...
% 位置误差状态单位，可选 "rad" 或 "m"。两种模式会分别保存结果。
...
for case_index = 1:numel(selected_cases)
    ...
    result_dir = fullfile(output_root, case_name, ...
        sprintf('four-method-comparison-%s', position_error_unit));
    ...
    try
        ...
        if overwrite_existing || ~first_stage_ready
            ...
            runtime_options = struct( ...
                ...
                'use_adaptive_forward_update', false);
            simulate_navigation_case( ...
                case_name, result_dir, false, false, runtime_options);
        ...
        state_unit = repmat(position_error_unit, height(case_statistics), 1);
        case_statistics = addvars(case_statistics, dataset, state_unit, ...
            'Before', 1, 'NewVariableNames', {'Dataset', 'PositionStateUnit'});
...
if ~isempty(all_statistics)
    summary_path = fullfile(exploration_artifact_dir(output_root), ...
        sprintf('four-method-comparison-%s-summary.csv', position_error_unit));
