...
```text
data/inertial-experiment/algorithm-exploration/navigation-results/simulation/<case>/fixed-lag-four-method-comparison-<rad|m>
```

每个数据集生成四条导航结果、旋转收缩诊断、固定滞后伪量测诊断、统一统计表，以及轨迹和水平径向误差对比图。跨数据集汇总保存为：

```text
data/inertial-experiment/algorithm-exploration/figures-tables/simulation/fixed-lag-four-method-comparison-<rad|m>-summary.csv
```
