...
enable_smoothing = 0;
enable_feedback = 1;
add_height_outlier = true;
height_outlier_step = 2000;
height_outlier_start = 1000;
height_outlier_bias_m = 5;
tic; % 启动计时器
for height_update_mode = [0, 1, 2]
    rng(1);
    %% 导入数据
    for smoothing_method = smoothing_methods
        ...
        % 构造范围数据
        range_stride = 420; % 距离文件为1 Hz，420行约为7 min
        for i = 1:3
        ...
        if add_height_outlier
            outlier_index = height_outlier_start:height_outlier_step:size(height, 1);
            height(outlier_index, 2) = height(outlier_index, 2) + height_outlier_bias_m;
            case_label = "abnormal";
        end
        ...
        switch height_update_mode
            ...
            case 1
                method_label = "assign";
            case 2
                method_label = "measUpdate";
            otherwise
                error("Unsupported height_update_mode: %d", height_update_mode);
        end

        navName = sprintf("Origin-rad-%s-%s.nav", method_label, case_label);
        navpath = fullfile(cfg.outputfolder, navName);
        navfp = fopen(navpath, 'wt');
        if navfp < 0
            error('无法创建高度对比导航结果：%s', navpath);
        end
        ...
        if enable_smoothing == 1
            ...
            navpath1 = fullfile(cfg.outputfolder, ...
                sprintf('Single-stage %s.nav', smoothing_method));
            navfp1 = fopen(navpath1, 'wt');
            % 二次跨区间 RTS 结果
            navpath2 = fullfile(cfg.outputfolder, ...
                sprintf('Proposed two-stage %s.nav', smoothing_method));
            navfp2 = fopen(navpath2, 'wt');
            if navfp1 < 0 || navfp2 < 0
                error('无法创建高度对比 RTS 结果：%s', cfg.outputfolder);
            end
        end
        ...
        for imuindex = 2:size(imudata, 1)
            ...
            if lastimu(1, 1) == rangedata(rangeindex, 1) && cfg.userange==1
                ...
                if height_update_mode == 1
                    navstate.pos(3) = height(imuindex,2);
                elseif height_update_mode == 2
                    % 调用高度卡尔曼量测更新
...
%% 对比本次生成的三种高度更新结果
nav_files = {
    fullfile(cfg.outputfolder, sprintf('Origin-rad-none-%s.nav', case_label))
    fullfile(cfg.outputfolder, sprintf('Origin-rad-assign-%s.nav', case_label))
    fullfile(cfg.outputfolder, sprintf('Origin-rad-measUpdate-%s.nav', case_label))
    };
method_names = ["无高度约束", "直接赋值", "高度量测更新"];
...
save_prefix = fullfile(artifact_dir, ...
    "height-update-comparison-" + case_label);
exportgraphics(comparison_figure, save_prefix + ".png", ...
    "Resolution", 300);
savefig(comparison_figure, save_prefix + ".fig");
writecell(radial_statistics, save_prefix + ".xlsx", "Sheet", "RMSE");

compare_height_vertical_rmse(cfg.truthpath, nav_files, ...
    'MethodNames', method_names, ...
    'OutputFile', fullfile(artifact_dir, ...
        "height-vertical-rmse-" + case_label + ".csv"), ...
    'FigureFile', fullfile(artifact_dir, ...
        "height-vertical-error-" + case_label + ".png"));
