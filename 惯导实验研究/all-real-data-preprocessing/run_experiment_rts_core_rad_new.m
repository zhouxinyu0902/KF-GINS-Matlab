function outputs = run_experiment_rts_core_rad_new(result_dir, runtime_options)
...
% 正式实验默认运行10000 s，当前数据时段内包含前23个测距点。
options.duration_s = option_value(runtime_options, 'duration_s', 10000);
...
artifact_dir = fullfile(cfg.outputfolder, 'artifacts');
...
if enable_fixed_lag_replay && ~any(fixed_lag_replay_is_processed)
    error('没有形成有效的二次RTS固定滞后区间，请检查测距点数量和时间范围。');
end
...
outputs = struct( ...
    ...
    'processed_range_event_count', processed_range_event_count, ...
    'range_original_time_s', original_range_times, ...
    'range_aligned_time_s', aligned_range_times, ...
    'range_alignment_offset_s', range_time_offsets, ...
