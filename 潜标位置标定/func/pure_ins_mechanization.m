function pure_ins_mechanization(cfg, imudata, truth)
glvs
%% 定义参数+加载过程配置
param = Param();
%% 加载数据
...
for imuindex = 2:size(imudata, 1)
    ...
    % 💡 修复：使用临时变量 z_depth 接收观测，不破坏原本的 heightdata 矩阵
    z_depth = heightdata(imuindex, 2) + randn * kf.depthstd;

    % 组装一个临时量测向量传给函数 [时间, 观测值]
    temp_depth_packet = [heightdata(imuindex, 1), z_depth];

    % 调用高度卡尔曼量测更新
    kf = myHeightUpdate(navstate, temp_depth_packet, kf);
