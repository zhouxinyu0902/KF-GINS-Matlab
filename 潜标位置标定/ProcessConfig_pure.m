...
function cfg = ProcessConfig_pure()
    param = Param();
    %% filepath
    cfg.imupath = 'D:\GitHub\KF-GINS-Matlab\潜标位置标定/imu.txt';
    cfg.outputfolder = 'D:\GitHub\KF-GINS-Matlab\潜标位置标定';
    % cfg.truthpath='D:\GitHub\KF-GINS-Matlab\惯导实验数据/input/truth_inte.txt';
    cfg.truthpath='D:\GitHub\KF-GINS-Matlab\潜标位置标定/truth.nav';
    %% configure
    ...
    cfg.initpos = [34;108;0.3]; % [deg, deg, m]
    cfg.initvel = [1.002; 0; 0]; % [m/s]
    cfg.initatt = [0.004;0.004;0.01]; % [deg]
    %
    % cfg.starttime = 2000;
    % cfg.endtime = cfg.starttime + 5000;
    % cfg.initpos = [34.00017214;107.99960307;-1004.265]; % [deg, deg, m]
    % cfg.initvel = [1.7321;-1.0000;0.0000]; % [m/s]
    % cfg.initatt = [0.004;0.004;330.01]; % [deg]
