...
%% 定义参数+加载过程配置
param = Param();
cfg = Config_10state('D:\Github\KF-GINS-Matlab\潜标位置标定\data2');
%% 加载数据
% imudata
...
switch dataType
    case 1
        ...
        % 动态输出对应的标准导航结果文件名
        navFileName = '/NavResult-origin.nav';
        xkFileName  = '/xk-origin.nav';
        
    case 2
        disp('>>> [数据流切换]: 载入【修正后】的标定补偿距离数据...');
        % 绑定刚才 range_reconstruct 导出的标定代偿文本
        rangedata1 = importdata([cfg.outputfolder, '/range_calib1.txt']);
        rangedata2 = importdata([cfg.outputfolder, '/range_calib2.txt']);
        rangedata3 = importdata([cfg.outputfolder, '/range_calib3.txt']);
        
        navFileName = '/NavResult-calib.nav';
        xkFileName  = '/xk-calib.nav';
        
    case 3
        disp('>>> [数据流切换]: 载入【真实的】潜标无误差距离数据...');
        % 绑定刚才 range_reconstruct 导出的绝对真实真值文本
        rangedata1 = importdata([cfg.outputfolder, '/range_true1.txt']);
        rangedata2 = importdata([cfg.outputfolder, '/range_true2.txt']);
        rangedata3 = importdata([cfg.outputfolder, '/range_true3.txt']);
        
        navFileName = '/NavResult-true.nav';
        xkFileName  = '/xk-true.nav';
        
    otherwise
        error('错误：未知的数据类型配置类型(dataType)，请在1~3之间选择。');
end
...
%%
path1 = 'D:\Github\KF-GINS-Matlab\潜标位置标定\data2\NavResult-origin.nav';
path2 = 'D:\Github\KF-GINS-Matlab\潜标位置标定\data2\NavResult-calib.nav';
path3 = 'D:\Github\KF-GINS-Matlab\潜标位置标定\data2\NavResult-true.nav';
