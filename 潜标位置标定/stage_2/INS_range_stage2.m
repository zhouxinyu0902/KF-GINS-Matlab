...
for dataType = 1:4
    switch dataType
        case 1
            ...
            rangedata1 = importdata([cfg.input, '/range1_stage2.txt']);
            rangedata2 = importdata([cfg.input, '/range2_stage2.txt']);
            rangedata3 = importdata([cfg.input, '/range3_stage2.txt']);
        ...
        case 2
            disp('>>> [数据流切换]: 载入【修正后】的标定补偿距离数据...');
            % 绑定刚才 range_reconstruct 导出的标定代偿文本
            rangedata1 = importdata([cfg.input, '/range1_stage2_calib_1.txt']);
            rangedata2 = importdata([cfg.input, '/range2_stage2_calib_1.txt']);
            rangedata3 = importdata([cfg.input, '/range3_stage2_calib_1.txt']);

            navFileName = '/Calib.nav';
            xkFileName  = '/xk-calib.nav';

        case 3
            disp('>>> [数据流切换]: 载入【二次修正后】的标定补偿距离数据...');
            % 绑定刚才 range_reconstruct 导出的标定代偿文本
            rangedata1 = importdata([cfg.input, '/range1_stage2_calib_2.txt']);
            rangedata2 = importdata([cfg.input, '/range2_stage2_calib_2.txt']);
            rangedata3 = importdata([cfg.input, '/range3_stage2_calib_2.txt']);
        ...
        case 4
            ...
            rangedata1 = importdata([cfg.input, '/range1_stage2_true.txt']);
            rangedata2 = importdata([cfg.input, '/range2_stage2_true.txt']);
            rangedata3 = importdata([cfg.input, '/range3_stage2_true.txt']);
...
%%
%%
path1 = [cfg.outputfolder,'\Origin.nav'];
path4 = [cfg.outputfolder,'\Calib.nav'];
path3 = [cfg.outputfolder,'\Calib-twice.nav'];
path2 = [cfg.outputfolder,'\True.nav'];
[fig,finalExcelData] = calc_radial_error(cfg.truthpath,path1,path2,path3,path4);
% 执行落地保存
outputExcelName = ['导航系统径向误差统计报告',trj,'-origin.xlsx'];
try
    writecell(finalExcelData, outputExcelName);
    fprintf('\n📊【数据成功导出】已自动将多系统径向误差对存储至: %s\n', outputExcelName);
catch
    warning('Excel 文件被占用，无法写入！请关闭「%s」后重试。', outputExcelName);
end
exportgraphics(fig, ['补偿前后误差对比',trj,'-origin.png'], 'Resolution', 600);
%%
path1 = [cfg.outputfolder,'\OriginLinear.nav'];
path4 = [cfg.outputfolder,'\CalibLinear.nav'];
path3 = [cfg.outputfolder,'\Calib-twiceLinear.nav'];
path2 = [cfg.outputfolder,'\TrueLinear.nav'];
[fig,finalExcelData] = calc_radial_error(cfg.truthpath,path1,path2,path3,path4);
% 执行落地保存
outputExcelName = ['导航系统径向误差统计报告',trj,'-smooth.xlsx'];
try
    writecell(finalExcelData, outputExcelName);
    fprintf('\n📊【数据成功导出】已自动将多系统径向误差对存储至: %s\n', outputExcelName);
catch
...
exportgraphics(fig, ['补偿前后误差对比',trj,'-smooth.png'], 'Resolution', 600);
