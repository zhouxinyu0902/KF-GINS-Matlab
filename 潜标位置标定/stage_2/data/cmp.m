...
for e = 1:numExp
    ...
    pathpos = fullfile(baseFolder, '/input_stage1/beacon_pos.mat');
...
filename = sprintf('三组实验_平均水平误差对比_%d.png', ID);

exportgraphics(gcf, fullfile(baseFolders{1}, filename), 'Resolution', 600);
...
filename = sprintf('三组实验_平均水平误差改善率对比_%d.png', ID);
...
filename = sprintf('三组实验_逐潜标水平误差对比_%d.png', ID);
