...
%% save result
navpath = [cfg.outputfolder, '/NavResult_ins_lbl'];
navpath = [navpath, '.nav'];
...
% end
LBLindex = 1;
while LBLdata(LBLindex, end) < thisimu(1, 1)
    LBLindex = LBLindex + 1;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for imuindex = 2:size(imudata, 1)-1
    ...
    if lastimu(1, 1) == LBLdata(LBLindex, end)
        ...
        % thisLBL = LBLdata(LBLindex, :)'+[rand*0.2/param.WGS84_RA,rand*0.2/param.WGS84_RA,rand*0.2,0]';% 加噪声
...
fclose(navfp);
% fclose(stdfp);
disp("LBL/INS Integration Processing Finished!");
%%
