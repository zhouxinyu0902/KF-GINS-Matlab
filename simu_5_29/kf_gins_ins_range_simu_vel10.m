...
%% save result
navpath = [cfg.outputfolder, '/NavResult_ins_range'];
navpath = [navpath, '.nav'];
...
rangedata = rangedata(1:3:end,:);
% 更改时间间隔
dtofRange = rangedata(2,1)-rangedata(1,1);
% 原始的heightdata采样时间0.2s
heightdata = heightdata(heightdata(:, 2) >= cfg.starttime, :);
heightdata = heightdata(heightdata(:, 2) <= cfg.endtime, :);
heightdata = heightdata(dtofRange/0.2:dtofRange/0.2:end,:);
%% 调试
