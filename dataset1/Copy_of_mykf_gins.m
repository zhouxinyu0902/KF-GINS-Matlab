% leador a15的惯导数据----不是很合适，在读取量测值的时候
% 比较适合用于仿真时候
% 15维的状态值和6维的噪声向量
clear;
% clc;
...
mode={'ins/gnss','ins/range','ins/2range','pureins'};
% 选择模式 %
chosenmode=mode{1};
%%%%%%%%%%%
switch(chosenmode)
    case 'ins/2range'
        ...
        rangeendtime2 = Rangedata2(end, 1);% 处理数据及时间
        pvapath=[cfg.outputfolder,'/ins','_2range.txt'];
        pvafp = fopen(pvapath,"wt");
        xkpath = [cfg.outputfolder, '/xk_2range.txt'];
        xkfp = fopen(xkpath, 'wt');% 设置输出结果的路径，包括导航结果和估计的xk，仅在没有反馈时有效
    case 'ins/range'
        depthdata = importdata(cfg.depthfilepath);
        depthstarttime = depthdata(1, 1);
        depthendtime = depthdata(end, 1);
