function rangedataget_2(path)
%% 不是三个信标，重新设置ADIS单信标或者其他情况
truth = importdata([path,'/truth.nav']);
GNSS_1s=truth(1:10:end,2:5);
%% 根据GNSS数据构造距离数据
t=length(GNSS_1s);
glvs
orgin0 = d2r(GNSS_1s(1,2:4));
dxyz_orgin0= [0, -5, 0] * 10;
...
distances= sqrt((trajectory_x - beaconxyz(1)).^2 + ...
        (trajectory_y - beaconxyz(2)).^2);

%%

beacon_repeat = repmat(rrm, length(trajectory_xyz), 1);
range=[GNSS_1s(:,1),distances(:,1),distances(:,1),beacon_repeat];
