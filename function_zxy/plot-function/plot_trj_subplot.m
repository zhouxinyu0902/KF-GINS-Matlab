function plot_trj_subplot(beacon, main_file, varargin)
% PLOT_TRJ 绘制多条轨迹对比图
% 输入:
...
%%
starttime=time_cell{i}(1);
endtime=time_cell{i}(end);
...
for i=1:num_files
    ...
    pos_cell{i} = pos_cell{i} (index0:indexend,:);
...
for i = 2:num_files
    subplot(3,3,i-1)
    plot(xyz_cell{1}(:,1), xyz_cell{1}(:,2),'r',...
        'DisplayName','truth');
    hold on
    plot(beacon(i-1,1),beacon(i-1,2),beacon_style{:})
    plot(xyz_cell{i}(:,1), xyz_cell{i}(:,2),'b')
    % 'DisplayName', get_file_label(all_files{i})
    title(['信标',num2str(i-1)]);
    xlabel('X坐标 (m)');
    ylabel('Y坐标 (m)');
end
